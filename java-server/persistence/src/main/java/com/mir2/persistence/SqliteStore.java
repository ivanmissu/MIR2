...
public final class SqliteStore implements AutoCloseable {
  ...
  public List<Character> characters(String account) {List<Character> result=new ArrayList<>();try(PreparedStatement p=connection.prepareStatement("SELECT id,name,job,level FROM characters WHERE account=? ORDER BY rowid")){p.setString(1,account);try(ResultSet r=p.executeQuery()){while(r.next())result.add(new Character(UUID.fromString(r.getString(1)),account,r.getString(2),r.getInt(3),r.getInt(4)));}return List.copyOf(result);}catch(SQLException e){throw failure(e);} }
  public void deleteCharacter(String account,UUID id) {try(PreparedStatement p=connection.prepareStatement("DELETE FROM characters WHERE account=? AND id=?")){p.setString(1,account);p.setString(2,id.toString());if(p.executeUpdate()!=1)throw new NoSuchElementException("character not found");}catch(SQLException e){throw failure(e);} }
