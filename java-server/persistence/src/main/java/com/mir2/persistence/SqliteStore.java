...
public final class SqliteStore implements AutoCloseable, com.mir2.auth.AccountStore, com.mir2.character.CharacterStore {
  ...
  @Override public void save(com.mir2.auth.Account account) { saveAccount(account.username(), account.passwordDigest()); }
  @Override public Optional<com.mir2.auth.Account> find(String username) { return passwordDigest(username).map(d -> new com.mir2.auth.Account(username, d)); }
  public void saveAccount(String username, byte[] digest) { require(username); try(PreparedStatement p=connection.prepareStatement("INSERT INTO accounts(username,password_digest,created_at) VALUES(?,?,?)")){p.setString(1,username);p.setBytes(2,digest.clone());p.setLong(3,System.currentTimeMillis());p.executeUpdate();}catch(SQLException e){throw failure(e);} }
  public Optional<byte[]> passwordDigest(String username) { try(PreparedStatement p=connection.prepareStatement("SELECT password_digest FROM accounts WHERE username=?")){p.setString(1,username);try(ResultSet r=p.executeQuery()){return r.next()?Optional.of(r.getBytes(1)):Optional.empty();}}catch(SQLException e){throw failure(e);} }
  ...
  @Override public List<Character> list(String account) {List<Character> result=new ArrayList<>();try(PreparedStatement p=connection.prepareStatement("SELECT id,name,job,level FROM characters WHERE account=? ORDER BY rowid")){p.setString(1,account);try(ResultSet r=p.executeQuery()){while(r.next())result.add(new Character(UUID.fromString(r.getString(1)),account,r.getString(2),r.getInt(3),r.getInt(4)));}return List.copyOf(result);}catch(SQLException e){throw failure(e);} }
  @Override public void delete(String account,UUID id) {try(PreparedStatement p=connection.prepareStatement("DELETE FROM characters WHERE account=? AND id=?")){p.setString(1,account);p.setString(2,id.toString());if(p.executeUpdate()!=1)throw new NoSuchElementException("character not found");}catch(SQLException e){throw failure(e);} }
  @Override public void close(){try{connection.close();}catch(SQLException e){throw failure(e);}}
