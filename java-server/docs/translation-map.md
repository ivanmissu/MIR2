...
| `Common/Grobal2.pas:734 TAbility` (50 bytes), `RM_ABILITY` | `gate/AbilityCodec` | packed little-endian layout with `MakeLong(min,max)` stat ranges; MaxExp and weight words left zero (reported via SM_WEIGHTCHANGED) |
...
| Magic, NPC scripts, item durability loss/repair | next/future slices | not started; the W12 slice closes the P2 item line up to equip/use/drop only |
