...
| Shadow comparison (same op stream, two servers) | Pass (Java↔Java) | `java-server/shadowdiff`: one deterministic op script drives two live servers over the real three-port protocol and diffs the player-observable snapshot after every op (STATE/ACKS = FAIL, MESSAGES = advisory unless `--strict-messages`). Strict 29/29 PASS in `g0-evidence/2026-09-21-shadowdiff-embedded-strict.md`; **PvE with monsters** now works too via world-seed injection (`MIR2_WORLD_SEED` / `--seed`, damage split off its own stream) — strict 17/17 PASS plus an unseeded negative control that correctly FAILs on 6 damage differences, in `g0-evidence/2026-09-22-shadowdiff-pve-seeded.md`. Both runs gate every PR in CI. Delphi↔Java needs only the Windows fixture (`--left-host <Delphi LoginGate>`, no code change) |
