...
> **W25 已交付（2026-09-23）：**接入层尾项三件（IP 黑名单 / `BlockMethod` / 包体突发守卫）全量落地，本地 **362/362 绿**（+32），含 7 个跨真实 TCP 的集成用例与一组负向对照。**Delphi 侧时间泵勘查已完成且结论为否定**：M2Server 有 **775 处裸 `GetTickCount`**（25 个单元），无可注入的缓存全局，驱动源 `svMain.dfm RunTimer(Interval=1)` → `svMain.pas:1069`，全局替换风险过高——故 G3 ② 的跨实现动态 AI 对拍**改走「墙钟但远离决策边界」路线**（W23 已在鸡上验证可行），不再等待 Delphi 时间泵。**下一次开发优先级**：①**Netty 接入替换**（接入层最后一块，W02 起挂账；当前 `GateServer` 为 thread-per-connection + 虚拟线程，替换前需先确认是否仍有必要）；②**env-lint / 配置校验收口**（本轮已为 9 个新变量加了 fail-fast 校验，可把同样的体检推广到全部 `MIR2_*`）；③**行会系统**（P3 独立候选，**动工前需重新确认范围**）；④W22 剩余尾项：客户端拒绝回包 `无法取下物品`(M2Share.pas:3157) 接线；⑤技能/魔法与 NPC 脚本引擎**红线仍未解除，不得开始**。G3 仍未正式过门：①3 名老玩家盲测、②Delphi↔Java 十怪执行，共同卡真实环境（Windows + mir2.exe + Delphi 服务端）。

> **上一步（W24，已完成）：**首批 10 种怪 AI 矩阵已从"逐条手工命令"固化为 `--ai-all` 一键入口，严格模式实跑 10/10 PASS（180 步零差异）且逐种证明 AI 非空转。其时列为下一步候选的「①Delphi 侧时间泵可行性勘查」已由 W25 完成并给出否定结论（见上）。
