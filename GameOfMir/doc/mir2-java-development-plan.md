...
*Development Plan · v1.0.28 · 2026-09-23*
...
> **W24 已交付（2026-09-23）：**首批 10 种怪 AI 矩阵已从“逐条手工命令”固化为 `--ai-all` 一键入口，严格模式实跑 10/10 PASS（180 步零差异）且逐种证明 AI 非空转。**下一次开发优先级**：① Delphi 侧时间泵可行性只读勘查，明确 G3 跨实现动态 AI 对拍走“时间泵”还是“墙钟但远离决策边界”路线；②若 Windows/Delphi 环境仍不可用，推进不受红线约束的接入层尾项（消息大小限制、IP 黑名单或 Netty 替换）/env-lint；③行会系统是 P3 可独立候选，但动工前需重新确认范围；④技能/魔法与 NPC 脚本引擎红线仍未解除，不得开始。G3 仍未正式过门：本轮是 Java↔Java 确定性回归，老玩家盲测与 Delphi↔Java 十怪执行仍卡真实环境。
...
*📋 MIR2 → JAVA · DEVELOPMENT PLAN v1.0.28**
