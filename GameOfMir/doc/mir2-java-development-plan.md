...
*Development Plan · v1.0.29 · 2026-09-23*
...
> **执行状态（截至 2026-09-23）**：S0/W01 协议基座已完成；W02 账号、角色、SQLite 持久化已完成；Gate 接入与会话路由已完成初版；可执行 JAR、环境配置、优雅停机及 Docker Compose 已交付。W03 的 **Tick、地图、碰撞、对象生命周期、12 格视野、RunLogin、移动协议及 GAME→World 接线** 已形成自动化闭环；近战攻击、单种怪物 AI、击杀/经验、掉落/拾取与战斗/背包状态落库已完成。W04 **物品目录与背包同步**已交付：完整 `StdItem` 模板与 SQLite `std_items` 目录、复刻 `GetItemNumber` 的稳定 `MakeIndex`、耐久字段、**76 字节 `TClientItem`** 小端编解码，以及 `CM_QUERYBAGITEMS → SM_BAGITEMS`（空包静默）与 `SM_ADDITEM` 完整载荷。W05 交付 **bot 压测军团**：50 机器人 × 5 分钟在 embedded 与 remote-fatjar 双模式均 PASS（0 错误），CI 每个 PR 增跑 50×2 分钟回归。本次会话（W06）交付 **traffic recorder / replayer 骨架**（P0 三件套至此齐活）：新增 `java-server/wiretool` 模块，`record` 透明代理把 `mir2.exe ↔ 服务端（Delphi 或 Java）`双向流量按 `#…!` 帧无损落盘 `.mrec`，`replay` 把录到的客户端帧按原节奏回放到目标服务端并与录制应答**逐字节对拍**（缺失/多余/首差异偏移分类，支持易变帧跳过清单与 `--structural-only` 结构级判定），`inspect` 输出逐帧注解（ident 名反查、GBK 正文、RunLogin 识别、cert 打码）；CI 新增 wiretool 门禁（代理实捕 bot 登录 → inspect 校验 → 结构级回放 PASS / 字节级回放因认证码易变而如期 FAIL）；dist 通道现同时发布 server/loadtest/wiretool 三个 fat JAR。本次会话（W07）交付**接入层加固第一项**：修复 W06 冒烟暴露的 `GateSessionRegistry.remove` 无调用点缺口——认证码在 GAME 登录真正进图成功后立即消费失效，GAME 连接断线（正常/异常）时同步清理 World 侧玩家对象与网关侧认证码；进图失败重试（如快速重登的 `leavePlayer` 竞态）仍可复用同一认证码直至真正进图成功，不影响 bot-swarm 既有的重登语义。真实 `mir2.exe` 对拍与 Delphi 实捕 golden 仍是最大缺口，待 Windows 客户端环境——届时直接用 wiretool 录 Delphi 链路并按字节级 golden 入库；连接数/频率限制与空闲超时已由 W08 交付。本次会话（W10）交付**门与传送点**：`.map` `btDoorIndex` 门锚点解析（同 index ±10 半径共享 `TDoorStatus`）、`CM_OPENDOOR → SM_OPENDOOR_OK` ±12 广播、500ms 扫拍 5 秒自动关门（`ProcessMapDoor` 语义）、「关门不挡路」客户端本地拦截 quirk 忠实复刻；经典 `MapInfo.txt` 加载器（`loadmapinfo` 子目录内嵌、`;` 注释、`[id|alias desc]` 条目、`GetValidStr3` 分隔符链路线行）与 `MIR2_MAPINFO_FILE` 多图模式；走/跑落格触发换图（`SM_CLEAROBJECTS→SM_CHANGEMAP→SM_MAPDESCRIPTION`、±1 关门压制连接点、目标不可走整步回滚）。城堡门差异分支、跨服切换、MapFlag 旗标与昼夜亮暗显式留在后续切片（MapFlag/昼夜已由 W11 交付）。本次会话（W12）交付 **物品切片：装备穿脱 + 使用 + 丢弃**，P2「物品」一条线就此收口：13 槽 `THumanUseItems` 装备容器、`CheckUserItems` 槽位匹配与 `CheckTakeOnItems` 穿戴条件（性别/负重/`Need` 族）、`RecalcAbilitys` 属性重算（`ApplyItemParameters` 按 `ItemType` 分支 + `Weight/WearWeight/HandWeight` 三桶负重 + `GetFeature` 外观联动）、`CM_TAKEONITEM/CM_TAKEOFFITEM/CM_EAT/CM_DROPITEM` 四条命令与 `SM_TAKEON_OK/FAIL`、`SM_TAKEOFF_OK/FAIL`、`SM_EAT_OK/FAIL`、`SM_DROPITEM_SUCCESS/FAIL`、`SM_SENDUSEITEMS`、`SM_WEIGHTCHANGED`、`SM_ABILITY` 应答，装备槽持久化（新表 `character_equipment`，旧库开库即原位升级）。**同时修复 master 编译中断**：W11（PR #25）在 CI 红灯状态下合入，`WorldEngine.java` 引用了 17 个不存在的方法，本次一并对齐既有 API 并补上缺失的 `addSpawner(MonsterSpawnDefinition)` 重载。本次会话（W14）交付 **等级提升 + 死亡/复活闭环**，P2「打怪闭环」一条线收口：`g_dwOldNeedExps` 经验表 + `GetLevelExp` 钳制、`RecalcLevelAbilitys` 三职业成长曲线（战士 DC 下限 1、仅战士有天然 AC 两处 quirk 忠实保留，Delphi 银行家舍入改用 `Math.rint`）、`GetExp/HasLevelUp` 升级循环（一次只扣一档、`MAXUPLEVEL` 封顶、升级即 `IncHealthSpell(2000,2000)` 回满）与 `SM_LEVELUP`；玩家死亡的 1/3 背包掉落惩罚（`nDieScatterBagRate=3`、`DropWide=2`）与 `SM_DELITEMS` 回推；`ReAlive` 原地复活 + `SM_ALIVE`、3 分钟 `MakeGhost` 尸体离场、重登 `HP<=0 → 14`；`TBaseObject.Run` 的 HP/MP 自然回复；GM `@Level`（`CmdChangeLevel`）。同时把 `Ability.defaultPlayer()` 从 W03 的 100HP/DC3-8 PoC 占位改为忠实的建号块（15HP/DC1-2/MaxExp 100），世界难度因此回到原版水平。本次会话（W15）交付 **修理 NPC 最小闭环 + 复活戒指 + 金币**：`TBaseObject.Run` HP=0 分支的复活戒指（`m_boRevival` 按 `RecalcAbilitys` 重算、60s `dwRevivalTime` 冷却、`ItemDamageRevivalRing` 每件扣 1000 耐久且无 break——双戒同耗、归零销毁清槽、SM_DURACHANGE 仅在千位桶变化时下发（银行家舍入 quirk：2500→1500 不发）、攻击者佩戴 Shape 144 `m_boUnRevival` 压制）；修理三件套 `CM_MERCHANTDLGSELECT`（`m_sScriptLable` 状态机，`@repair`/`@s_repair` 回 `SM_SENDUSERREPAIR` 开修理对话框，不引入脚本引擎）、`CM_MERCHANTQUERYREPAIRCOST → SM_SENDREPAIRCOST`（-1 = 不能修）、`CM_USERREPAIRITEM → SM_USERREPAIRITEM_OK(recog=金币,param=Dura,tag=DuraMax)/FAIL`，普通修理 `Dec(DuraMax,(DuraMax-Dura) div 30)` 后回满、特殊修理保持 DuraMax，价格公式 `Round(nPrice div 3 / DuraMax × 磨损)`（**quirk：特殊修理报价 3×截断基数 vs 实扣未截断基数，价格非 3 倍数时差 1-2 金币**，已按原版分别复刻）；金币 `m_nGold` 落库（W14 旧库原位升级默认 0）、`SM_ABILITY` 头部补 recog=m_nGold/param=MakeWord(btJob,99)（此前恒 0 会清掉客户端金币显示）、`SM_GOLDCHANGED`、`MIR2_TEST_GOLD` 登录下限（复刻 boTestServer/nTestGold，默认 0 = no-op）。本次会话（W16）交付 **影子对拍 harness（P2 任务表最后一个未启动项，G3 硬性要求）**：新增 `java-server/shadowdiff` 模块（shaded `mir2-shadowdiff.jar`，dist 通道第四 JAR）——同一份确定性操作脚本（turn/walk/run/hit/pickup/bag/say/drop/eat/takeon/takeoff/sleep/relog，一行一 op）经真实三端口线上协议依次驱动两台服务端，每 op 后按静默窗口排干应答并 diff 玩家可观测状态快照（地图/坐标/朝向/HP/MP/等级/金币/背包/装备，MakeIndex/对象 id 归一化）；判定三级：状态差异/应答差异 = FAIL、消息集合差异默认仅提示（`--strict-messages` 升级）；`--embedded` 进程内双 Java 自拍（骨架自检 + 确定性回归，严格判定 29/29 步 PASS 已入库 g0-evidence）、remote 模式对拍任意两台已运行服务端——Delphi 环境就绪后指向 Delphi LoginGate 即为 G3 对拍，无需改代码；CI 增 shadowdiff-smoke 每 PR 门禁；本地全量 222/222 绿（204 → +18）。P2 任务表至此全部收口，G3 剩余两项（盲测手感、10 怪对拍）仍卡真实客户端环境。本次会话（W18）交付 **真实 StdItems.DB / Monster.DB 数据导入**（计划内「性价比最高的下一件」，纯数据、不受红线约束）：以官方 1.76 GEEM2 基线转储（`cjlaaa/Mir2-GeeM2`，与 W09 外观校正同源）为唯一数据源，新增 `java-server/scripts/extract-geem2-db.py` 生成受管的 UTF-8 数据工件——`db/MonsterDb.tsv`（378 行全表）、`db/StdItemsDb.tsv`（686 行全表，两个上游重名按 Delphi 首行命中收敛为 684 项）、`db/MonItems/`（10 张引擎已支持怪物的掉落表 + 名称索引）；world 新增 `MonsterDb` / `StdItemsDb` / `MonsterDropTable` 三个加载器，`MonsterTemplate` 首批 10 种怪 + 木桩全部改为按 `UsrEngn.pas:2581` 字段映射从 DB 行构建（HP/DC/AC/等级/经验/走攻间隔，含 `LoadMonsterDB` 的 200ms 下限钳制与 AC `MakeLong(wAC,wAC)` 平直值语义），掉落取自真实 MonItems 行（金币行解析后延期到金币地面堆切片，`金币/鹿肉` 之外的全部名称均可在真实 StdItems 中解析——原 W03 占位掉落 `鹿肉` 由真实掉落 `肉(1/1)×2 + 鹿血(1/10000)` 取代）；`StdItems.defaults()` 由 5 件手挑目录升级为 684 项权威全量导入（W04/W12/W15 的全部物品数值占位与 TODO(verify) 就此消除，含 `木剑` DuraMax 4000/DC 2-5/等级 1/价 50、`鸡肉` StdMode 40 肉类、`复活戒指` AC/MAC 0-1 等真值），SQLite `std_items` 启动种子自动跟随。新增 `Geem2BaselineImportTest`(5) 钉死模板↔DB 行↔掉落表全字段对齐，7 处既有断言（战斗节拍/经验值/物品字段）按权威数值更新；**本地全量 253/253 绿**（基线 248 → +5）。本次会话（W19）交付 **`TStdItem.Reserved` 独立字节还原**：W18 导入时被并进 `NeedIdentify` 的 `RESERVED` 列重新拆为独立字段，SQLite `std_items` 原位迁移，并复刻 `Reserved & 2`（未喝解锁药时锁定）/`Reserved & 4`（无条件锁定）两条取下禁制 quirk（本地 259/259 绿）。本次会话（W20）交付 **`DropUseItems`（死亡掉装备）+ PK 等级/名字颜色模型 + 红名全掉**，把 W14/W15 起挂账最久的死亡惩罚缺口一次补齐：`TPlayObject.DropUseItems` 两趟扫槽（`Reserved and 8` 原地销毁并以**空名**回报、余槽 1/30 掉落且红名 1/15、`Reserved and 10 <> 0` 时「掉在地上却仍然穿着」的复制 quirk 照搬）、`RecalcAbilitys` 的 `m_boAngryRing/m_boNoDropItem/m_boNoDropUseItem` 三面护身旗（槽位读 AniCount 还是 Shape 的不对称保留）、`PKLevel = m_nPkPoint div 100` 与 `GetNamecolor/GetCharColor` 名字颜色（$FB 黄名 / $F9 红名 / $2F 交手染色，且交手色只在红名以下压过等级色）、`IncPkPoint/DecPKPoint` 两分钟衰减（**只有旧等级为 1 或 2 时才重绘**的广播 quirk）、`SetPKFlag` 60 秒交手标记、`Die` 谋杀分支 +100 点与两句 GBK 文案（受害者已挂 PK 旗则判正当防卫）、`boDieRedScatterBagAll` 红名全掉；顺带改正 `ScatterBagItems` 长期误用 `NOTHROWITEM` 的地图旗（真正的旗是 `NODROPITEM`，`MapFlags`/`MapInfoLoader` 补该旗解析），并把 `character_state.pk_point` 原位加列落库。**本地全量 276/276 绿**（基线 259 → +17）。W21（PR #39 合入 master）交付 **NPC 最小切片可见半 + 真机四联修复**：`spawnNpc` 装饰性 NPC 站位（`MIR2_NPC_LIST`/默认老兵·老板·商人）、`CM_SOFTCLOSE`、`SM_CHANGELIGHT`、`reconcileStandardItems` 目录对账，基线抬到 283/283。本次会话（W22）交付 **补齐 W20 遗留小项**：`world/BodyLuck` 幸运值累加器（`AddBodyLuck` 非对称守卫 + 经验/升级/死亡/谋杀全链路，`Trunc` 派生等级钳制 `[-10,5]`）、`world/WeaponPoints` 武器幸运/诅咒双字节（`MakeWeaponUnlock` 先耗幸运再累积诅咒上限 10、仅经 `GetItemAddValue` 折进客户端 `TClientItem` 不入服务端战斗）、`world/DisableTakeOffList`（`InDisableTakeOffList`/`LoadDisableTakeOffList` 按名匹配，门禁 `ClientTakeOffItems` 与 `DropUseItems` 跳槽，`MIR2_DISABLE_TAKEOFF_FILE` GBK 装载并推入世界线程）、`GetAttackPower` 幸运暴击接入玩家近战（**luck==0 逐位复刻旧 `randomBetween`、确定性向量零回退**），三列持久化（`body_luck`/`weapon_luck`/`weapon_curse` 经 `ensureColumn` 原位加列），`WorldRandom.Stream` 增第 7 条流 `WEAPON_UNLOCK`。**本地全量 307/307 绿**（基线 283 → +24）。本次会话（W23）交付 **虚拟时钟注入 + 会动的怪确定性对拍**（需求方本轮显式指定）：新增 `world/WorldClock`（`SYSTEM`/`VIRTUAL`/`MANUAL` 三态）把 Delphi 遍布全服的 `GetTickCount`（`ObjBase.pas` 310 处、`UsrEngn.pas` 87 处、`ObjMon.pas` 79 处）收敛为一个可注入的时间源——`VIRTUAL`/`MANUAL` 下世界时间为 `epoch + ticks × tickMillis`，怪物走位（ObjMon.pas:437）/攻击（ObjMon.pas:392）/刷怪/回复/PK 衰减/昼夜全部变成 tick 计数的纯函数，从而补上 `WorldRandom`（W17）留下的另一半缺口（种子只定「第 N 次抽签抽什么」，定不了「到底抽了几次」），**对拍从此不再局限于木桩**；`WorldEngine` 拆出 `runTickBody()`、新增 `advanceTicks(int)` 推进泵与 `@tick [N]` harness 专用 GM 命令（无 Delphi 对应物，仅 MANUAL 世界接受、上限 10 万 tick、回执带新世界时间），MANUAL 下 `tickOnce()` 跳过周期体（命令排空仍无条件执行）以消除「怪物这一拳落在哪个 op 观测桶」的调度竞态；`ServerConfig` 增 `MIR2_WORLD_CLOCK`（`system` 默认 / `virtual`，**`manual` 被环境解析显式拒绝**）；shadowdiff 增 `--ai`（MANUAL 时钟 + 4 只鸡 + `Op.aiScript()`）、脚本 op `tick <N>`、快照 `near=`/`worldTime=`、负向对照开关 `--right-seed`、以及**「观测轨迹」报告表**（逐 op 打印左侧状态，此前 PASS 无法审计——分不清「0 差异」与「空转」）。实跑：`--ai --strict-messages` 18 步 PASS 且轨迹证明怪物真在追击/攻击（HP 15→8）、玩家真在反击（dmg=2）、relog 后普查逐位一致；负向对照 `--right-seed 99999` 如期 FAIL 并定位到 `#12 hit 6` dmg 2 vs 1。**关键对照实验**：把 AI 决策构造到恰好落在 op 观测边界（sleep 值 = 鸡的 3000ms 攻击间隔）时，墙钟版 **5/5 FAIL 且差异数逐次漂移**，等价的 tick 版 **5/5 PASS**——虚拟时钟的价值不在「随便什么场景都更稳」（诚实记录：常规脚本下墙钟版空载/满载/收紧静默窗口均 PASS，因鸡的节拍远大于本机抖动），而在**消除决策落在观测边界时的桶归属竞态**。**本地全量 329/329 绿**（基线 307 → +22）。本次会话（W24）把 W23 的单怪入口扩成 **首批 10 种怪 AI 确定性对拍矩阵**：shadowdiff 新增 `--ai-all`，显式钉死 P2 已入库的 chicken/deer/scarecrow/hookcat/rakecat/cavemaggot/scorpion/orc/orcwarrior/orcfighter 十种模板（不动态枚举，防止越过剩余 47 怪红线），每种使用隔离双服/SQLite、MANUAL 时钟和同一随机种子，失败行不短路，输出十份详细报告与 `ai-matrix.md` 汇总。实跑 `--embedded --ai-all --strict-messages --seed 20260923`：**10/10 PASS、180 个观测步骤、状态/应答/消息差异均为 0**；每种均有 3–4 种不同 `near` 轨迹，攻击型怪均产生玩家受击/死亡，鹿则持续远离玩家，排除空转。证据见 `java-server/docs/g0-evidence/2026-09-23-w24-first-ten-ai-matrix.md`。这仍是 Java↔Java 确定性回归，不替代卡在 Windows/Delphi 环境的跨实现 G3 签发。
...
| ✅ 完成（本地闭环） | W16：影子对拍 harness（P2 表内最后一个未启动项，G3 硬性要求） | 新增 `java-server/shadowdiff` Maven 模块（shaded `mir2-shadowdiff.jar`）：**同一份确定性操作流驱动两台服务端并逐步 diff 玩家可观测状态**。① `Op` 操作脚本方言（一行一 op、`#` 注释）：`turn/walk/run/hit/heavyhit/bighit <dir 0..7>`、`pickup`、`bag`、`say <文本>`、`drop/eat/takeon/takeoff <物品名>`、`sleep <ms>`（固定节奏，保证动作间隔判定两边一致）、`relog`（存档往返）；`describe()` 与解析器互为逆运算，内置 29 步冒烟脚本（含 relog）；② `ShadowWireClient` 说与 bot-swarm 完全一致的线上方言（`#…!` 帧 + 轮转序号、RunLogin 首包、`+GOOD/+FAIL` 原始应答；loadtest 侧同名类为包私有故此处独立实现）；③ `ShadowSession` 单线程同步驱动：三段式登录（`CM_PROTOCOL→CM_IDPASSWORD→CM_SELECTSERVER→CM_QUERYCHR→[CM_NEWCHR]→CM_SELCHR→RunLogin`）、每 op 发送后按**静默窗口**（默认 400ms）排干应答归桶，从线上包重建 `StateSnapshot`（地图/坐标/朝向/HP/MP/等级/金币/背包/装备；**MakeIndex 与对象 id 归一化**——两台服务端各自编号，物品身份取 `名称 + Dura/DuraMax` 排序集合）；解析 50 字节 `TAbility`、76 字节 `TClientItem`、`SM_SENDUSEITEMS` 分槽体、`SM_DELITEMS` 名/MakeIndex 对；④ `ShadowDiff` 三级判定：**STATE（状态快照差异）> ACKS（+GOOD/+FAIL 序列差异）= FAIL**，MESSAGES（服务端消息**多重集合**差异，顺序归一化——兴趣广播交错是合法的）默认仅提示、`--strict-messages` 升级 FAIL；两侧步数不一致直接判 harness 错误；⑤ `ShadowReport` 中文 Markdown + CSV（逐步判定表 + 差异明细 + 脚本原文），退出码即结论；⑥ CLI 双模式：`--embedded` 进程内两台独立 `Mir2Server`（独立 SQLite/端口、相同配置）自拍 = 骨架自检 + Java 确定性回归；remote 模式 `--left-host/--right-host` 对拍任意两台已运行服务端——**Delphi 环境就绪后 `--left-host` 指向 Delphi LoginGate 即为 G3 要求的 Delphi↔Java 对拍，无需改代码**；⑦ CI：`shadowdiff-smoke` job 每 PR 跑一轮 embedded 自拍（宽松消息判定——顺序运行的两个会话可能跨昼夜边界，SM_DAYCHANGING 只落一侧），dist 通道第四个 fat JAR；⑧ 边界：embedded 双世界不放怪（两台服务端各自独立 Random，PvE 战斗结果对拍需要注入种子的后续切片——挂怪时对拍 PvE 伤害数值属已知限制，移动/物品/存档/聊天不受影响） | `shadowdiff/pom.xml`、`Op.java`、`WireTarget.java`、`ShadowWireClient.java`、`ShadowSession.java`、`StateSnapshot.java`、`OpObservation.java`、`IdentNames.java`（ProtocolConstants 反射反查）、`ShadowDiff.java`、`ShadowReport.java`、`ShadowDiffMain.java`；新增 `OpTest`(6) + `ShadowDiffTest`(7) + `ShadowReportTest`(2) + `ShadowSessionEmbeddedTest`(3，真 TCP 双服)，**本地全量 222/222 绿**（基线 204 → +18）；embedded 自拍严格判定 PASS（29/29 步一致，报告入库 `g0-evidence/2026-09-21-shadowdiff-embedded-strict.md`），故意错开出生点的发散检测 FAIL 如预期 |
| ✅ 完成（本地闭环 + 实跑证据） | W17：世界随机种子注入 + 带怪 PvE 影子对拍（解除 W16 已知限制） | ① **`WorldRandom`**（world 新增）：复刻 Delphi 全服共用一个全局 `Random()` 的行为为 `of()`（未设种子 = 一条共享流，生产默认，语义与原版一致），另提供 `seeded(seed)` 把随机性按用途拆成 **5 条互不干扰的流**——`DAMAGE`/`EQUIPMENT_WEAR`/`LOOT_DROP`/`DEATH_SCATTER`/`SPAWN`，每条由世界种子经 **SplitMix64 雪崩**派生（避免相邻流相关）。**关键收益：掉落 roll 了几次不再影响第 N 次伤害**——Delphi 里一次掉落抽样会挪走本该属于下一次伤害的抽样，生产无所谓但对拍致命（两进程墙钟不同 ⇒ 抽样次数不同 ⇒ 后续数值全错位）；② `WorldEngine` 9 处随机点全部改走 `WorldRandom` 对应流，新增 `worldSeed()`；`ServerConfig` 增 `MIR2_WORLD_SEED`（未设 = 生产随机，等价原版）；③ **`MonsterBehavior.STATIONARY` + `MonsterTemplate.trainer()`（木桩）**：忠实复刻 `TMonster.Run` 中 **`if not m_boNoAttackMode`（ObjMon.pas:449）整块包住「索敌→追击→攻击」**的语义——tick 循环开头直接 `continue`。溯源 `TRAINER=55`（M2Share.pas:152）→ `TTrainer`（ObjNpc.pas:2626）官方伤害测试木桩；Delphi 侧它是 NPC，本次因 **NPC/脚本引擎红线未解除**，改用 monster 对象实现等价线上可观测行为（站立、挨打、死亡、不掉落），**不引入任何脚本钩子**；木桩是**唯一行为与墙钟无关**的 PvE 目标，故可确定性对拍（会动的怪仍会因墙钟差异发散，属墙钟问题而非随机数问题）；④ `shadowdiff` 扩观测面：`StateSnapshot` 增 `experience` 与 `combat` 两字段，`ShadowSession` 新解析 `SM_STRUCK`（伤害/HP 归一化 self/other）、`SM_DEATH`、`SM_WINEXP`，`ShadowDiff` 增 `exp:`/`combat:` 两项比较——**此前挨打掉血根本不在对拍视野内**；⑤ CLI 增 `--pve`（内置 PvE 脚本：走位贴桩 + 6 连击含重击/刺杀 + relog 存档往返）、`--monsters N`、`--monster-kind NAME`、`--seed N`（两侧共用；remote 模式两台服务端各配同一 `MIR2_WORLD_SEED`）；⑥ **负向对照证据**：同脚本同地图同 8 木桩，仅种子不同 → unseeded **FAIL（6 处状态差异，`dmg=1` vs `dmg=2`、木桩 HP 发散）**、seeded 同种子 **PASS（0 差异）**，证明对拍非空转；⑦ CI 增第二道 shadowdiff 门禁 `--pve --seed`。**注意**：木桩 HP/AC 为 `TODO(verify)` 占位，待 `Monster.DB` 第 55 行导入校准 | `world/WorldRandom.java`（新增）、`WorldEngine.java`、`MonsterBehavior.java`、`MonsterTemplate.java`、`bootstrap/ServerConfig.java`、`Mir2Server.java`、`shadowdiff/{StateSnapshot,ShadowSession,ShadowDiff,Op,ShadowDiffMain}.java`、`loadtest/LoadtestMain.java`（14 参构造器对齐）；新增 `WorldRandomTest`(6)+`WorldSeedDeterminismTest`(5)、`ServerConfigTest` +2、`ShadowSessionEmbeddedTest` +2（真 TCP 双服带怪 PvE，含异种子发散检测），**本地全量 237/237 绿**（基线 222 → +15）；实跑证据入库 `g0-evidence/2026-09-22-shadowdiff-pve-seeded.md`（严格判定 17/17 步 PASS + unseeded 负向对照 FAIL） |
| ✅ 完成（本地闭环） | W18：真实 StdItems.DB / Monster.DB 导入（W17 交接建议①，纯数据、不受红线） | ① **数据源与提取器**：官方 1.76 GEEM2 基线转储（`cjlaaa/Mir2-GeeM2` main @9981a8d7，与 W09 RaceImg/Appr 校正同源），新增 `java-server/scripts/extract-geem2-db.py`——从 `GEEM2.db.sql` 内存级解析 Monster(378)/StdItems(686) 原生列、从 `Envir/MonItems/*.txt`（10 张全 UTF-8，上游 GBK→UTF-8 转换损坏仅限注释行）解析掉落行，确定性生成受管工件并内置防呆（名称必须 ≤20 GBK 字节、重名首行获胜、非 1 分子告警、不可解析名打 DEAD-ROW 注记）；② **受管数据工件**：`world/src/main/resources/db/MonsterDb.tsv`（全 378 行 + 溯源头）、`StdItemsDb.tsv`（全 686 行，重名「魔法头盔/火龙气焰」保留两行、按名收敛首行）、`MonItems/`（ascii 文件名 + `index.tsv` 名称索引——中文资源名在 C locale JVM 下会因 `sun.jnu.encoding=ANSI` 无法寻址，故作名称→文件间接层）；③ **加载器与接线**：`MonsterDb`（22 列全字段记录 + `clampActionInterval`（200ms 下限，复刻 `LoadMonsterDB`）+ `monsterWeapon=LoByte(MP)`）、`StdItemsDb`（23 原生列，`Ac/Ac2…Sc/Sc2` 打包 Delphi dword，`RESERVED→needIdentify`，首行获胜收敛）、`MonsterDropTable`（`num/den name count` 行 → `ItemDrop`（looks 取自真目录），`金币` 行解析为 `GoldDrop` 延期（金币地面堆/拾取入钱包属 P3 经济切片），Delphi `FileExists` 语义：无表怪物（木桩）返回空表）；④ **模板与目录重写**：`MonsterTemplate` 首批 10 种 + 木桩全量改为 `fromDb`（`UsrEngn.pas:2581` 映射——HP=MaxHP=wHP、AC=`MakeLong(wAC,wAC)` 平直值、DC=MakeLong(DC,DCMAX)、Level/Exp、WALK_SPD/ATTACK_SPD 钳制后作走攻间隔；视野范围与行为族仍为代码侧常数 TODO(verify)），掉落换成真实 MonItems（`鸡` 鸡肉1/1、`鹿` 肉1/1+肉1/10+鹿血1/10000、`洞蛆` 蛆卵1/5+9 种特戒各 1/1000 万；**W03 占位掉落 `鹿肉` 整体退役**——真实 StdItems 只有 `肉`），木桩经 `练功师` 行校准 HP 9999/AC 0/Exp 1（TODO(verify) 消除）；`StdItems.defaults()` 升级为 684 项权威全量导入（W04 5 件占位与全部 TODO(verify) 消除——`鸡肉` StdMode 40/DuraMax 4000，`木剑` Shape 1/Weight 4/DuraMax 4000/DC 2-5/NeedLevel 1/价 50，`金创药(小量)` AniCount 102/Looks 398/价 40，`复活戒指` AC/MAC 0-1/Looks 175/NeedLevel 16/价 20000），SQLite `std_items` 启动种子自动跟随（INSERT OR IGNORE 逐行）；⑤ **测试**：新增 `Geem2BaselineImportTest`(5) 钉死模板↔DB↔掉落全字段、重名首行获胜、解析器拒绝坏行；7 处既有断言按权威数值更新（`木剑` 负重/字段、鸡肉 Looks 13、鸡经验 9 → 升级 12 杀结存 8、半兽人攻击间隔 2500ms → 三处时间推进改以模板 `attackIntervalMillis()` 推导、占位名「屠龙」因真实存在改为「根本不存在之剑」）；**本地全量 253/253 绿**（248 → +5） | `java-server/scripts/extract-geem2-db.py`、`world/resources/db/{MonsterDb,StdItemsDb}.tsv`、`world/resources/db/MonItems/`(10+index)、`world/MonsterDb.java`、`world/StdItemsDb.java`、`world/MonsterDropTable.java`、`world/MonsterTemplate.java`（fromDb）、`world/StdItems.java`（导入门面）、`SqliteStore.java`（种子跟随）；新增 `Geem2BaselineImportTest`(5)；**本地 253/253 全绿**（ECJ+jdk4py 链路）。红线核对：未新增怪物行为（57 怪红线）、未触碰技能/魔法与 NPC 脚本；`金币` 掉落行仅解析未落地（P3）；Magic 表未导入（红线） |
| ✅ 完成（本地闭环） | W19：`TStdItem.Reserved` 独立字节还原 + 取下锁定 quirk（PR #37） | W18 导入时把 `RESERVED` 列并进了 `NeedIdentify`，本轮把它拆回 `StdItem.reserved` 独立字段；`StdItemsDb`/`SqliteStore` 原位迁移 W18 旧缓存；复刻 `ClientTakeOffItems`（ObjBase.pas:17238-17262）的两条取下禁制——`m_boUserUnLockDurg = False`（登录默认）时 `Reserved & 2` 锁定、`Reserved & 4` 无条件锁定；祈祷/赤血类标志在 `TClientItem` 线上保真 | `world/StdItem.java`、`world/StdItemsDb.java`、`persistence/SqliteStore.java`、`WorldEngine.isLockedInPlace`；**本地 259/259 全绿**（253 → +6） |
| ✅ 完成（本地闭环） | W20：`DropUseItems`（死亡掉装备）+ PK 等级/名字颜色模型 + 红名全掉（W18/W19 交接建议③，挂账自 W14） | ① **`TPlayObject.DropUseItems`**（ObjBase.pas:15487）两趟扫十三槽：**趟一** `StdItem.Reserved and 8` 原地销毁、**永不落地**、且以 `DelList.AddObject('', MakeIndex)` 的**空名**回报（客户端只认 MakeIndex）——因 `StdItem` 紧凑构造器禁止空名，新增 `world/ItemRemoval`（`(name, makeIndex)` 记录，允许空名）承载 `SM_DELITEMS` 条目，`WorldEvent.ItemsRemoved` 随之改签名；**趟二** 每槽 `Random(nRate) = 0`，`nDieDropUseItemRate` 30、`PKLevel > 2` 时 `nDieRedDropUseItemRate` 15，中签者 `DropItemDown(...,2,True)` 落地，但**只有 `Reserved and 10 = 0` 才清槽**——`Reserved and 2` 的物品「掉在地上却仍然穿着」的复制 quirk 原样照搬；② **门禁链**：`m_boAngryRing or m_boNoDropUseItem` 先行退出，`Die`（ObjBase.pas:21002）再按 `boKillByMonstDropUseItem`(True)/`boKillByHumanDropUseItem`(False) 判定凶手——**怪物杀你掉装备、玩家杀你不掉**，无归属击杀一律掉；**`DropUseItems` 没有地图门禁**（`Die` 自身的 `(not m_boNoItem) or (not Flag.boNODROPITEM)` 是个 OR，纯 NODROPITEM 图满足不了），忠实保留；③ **`world/DropProtection`** 复刻 `RecalcAbilitys` 的三面护身旗 `m_boAngryRing`（两项惩罚全免）/`m_boNoDropItem`（只保包裹）/`m_boNoDropUseItem`（只保装备），**槽位不对称保留**：武器/右手/衣服读 `AniCount`、其余槽读 `Shape`，码值 170/171/172，经典 117 形式只在手/衣分支与两戒指槽生效，0 耐久不生效；④ **`world/PkLevel`**：`PKLevel = m_nPkPoint div 100`，颜色 255 白 → $FB 黄名 → $F9 红名，**顺序 quirk**：`m_boPKFlag` 的 $2F 交手色压过等级色、但只在 `PKLevel < 2` 时（红名交手仍是红名）；⑤ **计数器与广播**：`IncPkPoint` 一次谋杀 +100（`nKillHumanAddPKPoint`），`TBaseObject.Run` 每 `dwDecPkPointTime` 2 分钟 `DecPKPoint` 掉 1 点，**广播 quirk**：等级变化且**旧等级为 1 或 2** 时才 `RefNameColor`——深红从 3 掉到 2 全程不重绘；`SetPKFlag`（ObjBase.pas:21220）交手把**攻击方**染色 `dwPKFlagTime` 60 秒（整块以受害者 `not m_boPKFlag` 为前置，任一方红名或身处 FIGHT 区则跳过），`CheckPKStatus` 到点清除；`RefNameColor → RM_CHANGENAMECOLOR(8090) → SM_CHANGENAMECOLOR(656)` 经新事件 `WorldEvent.NameColorChanged` 下发给全体观察者与本人；⑥ **谋杀分支**（ObjBase.pas:20938）按出厂配置（`boKillHumanWinLevel/Exp` 皆 False、无行会战、无沙城）：凶手 +100 点并双方收到 GBK 文案「你犯了谋杀罪...」/「你被%s杀害了...」；受害者已挂 PK 旗 → `IsGoodKilling` 只发「[--你受到正当规则保护--]」且不计点；FIGHT 区或受害者已红名则整段跳过；`AddBodyLuck(-500)` 与 `MakeWeaponUnlock` 缺幸运/武器锁模型，注释标注延后；⑦ **`boDieRedScatterBagAll`**：`PKLevel >= 2` 死亡时包裹**全掉**而非 1/3；⑧ **顺带纠错**：`ScatterBagItems` 长期误用 `Flag.boNOTHROWITEM` 当豁免旗，真正的旗是 `Flag.boNODROPITEM`（Delphi 里是两个不同旗：前者禁丢、后者禁死亡掉落），`MapFlags`/`MapInfoLoader` 补 `NODROPITEM` 解析；`WorldRandom.Stream` 增第 6 条流 `DEATH_DROP_USE_ITEM`（装备掉落不再扰动包裹掉落序列）；⑨ **持久化**：`character_state.pk_point` 原位加列（旧库默认 0），`PlayerState` 增第 6 分量（旧构造器保留兼容重载）；`m_boPKFlag` 按 Delphi 语义**不落库** | `world/PkLevel.java`、`world/DropProtection.java`、`world/ItemRemoval.java`（均新增）、`world/WorldEngine.java`（`dropUseItems`/`applyMurderPenalty`/`setPkFlag`/`decayPkPoints`/`broadcastNameColor`/`addPkPoint`/`pkPoint`）、`world/WorldEvent.java`、`world/PlayerState.java`、`world/MapFlags.java`、`world/MapInfoLoader.java`、`world/WorldRandom.java`、`persistence/SqliteStore.java`、`gate/GameProtocolAdapter.java`；新增 `WorldPkAndEquipmentDropTest`(17)；**本地全量 276/276 绿**（259 → +17）。红线核对：未触碰技能/魔法、NPC 脚本引擎与 57 怪行为扩展。剩余延后项：`InDisableTakeOffList`（服务器配置文本清单，非目录列）、`AddBodyLuck(-500)`、`MakeWeaponUnlock`、行会/沙城/FIGHT3 的颜色分支 |
| ✅ 完成（PR #39 合入 master） | W21：NPC 最小切片（可见半） + 真机四联修复 | ① **`spawnNpc`**：`WorldEngine` 新增装饰性 NPC 生成（`TNormNpc` 站位替身，出生点附近按 `MIR2_NPC_LIST`=`name:appearance:dx:dy` 摆放，格子被占/越界即跳过并告警——复刻 `AddNpc` 对不可走格的处理），对话/交易仍在 `Market_Def` 红线之外；②`CM_SOFTCLOSE`、`SM_CHANGELIGHT`（昼夜亮度）、`reconcileStandardItems`（`std_items` 目录对账）、`MIR2_NPC_LIST` 解析与默认 NPC（老兵/老板/商人）；③`ServerConfig.NpcPlacement` 记录 + `Mir2Server.spawnNpcs` 装载 | `world/WorldEngine.java`（`spawnNpc`）、`bootstrap/Mir2Server.java`（`spawnNpcs`）、`bootstrap/ServerConfig.java`（`MIR2_NPC_LIST`/`NpcPlacement`）、`gate/GameProtocolAdapter.java`、`persistence/SqliteStore.java`（`reconcileStandardItems`）；**基线 283/283**。红线核对：NPC 仅可见站位，未触碰脚本引擎 |
| ✅ 完成（本地闭环） | W22：补齐 W20 遗留小项（`InDisableTakeOffList` + `AddBodyLuck` 幸运模型 + `MakeWeaponUnlock` 武器锁） | ① **`world/BodyLuck`**：`m_dBodyLuck` 幸运值累加器 + 派生等级（`Trunc(value/BODYLUCKUNIT)` 钳制 `[-10,5]`），复刻 `AddBodyLuck`（ObjBase.pas:2374）的**非对称守卫**（正增量仅在 `<5*UNIT` 生效、负增量仅在 `>-5*UNIT` 生效，单次可略越界但不复利）；经验 `AddBodyLuck(dwExp*0.002)` + 升级 `AddBodyLuck(100)` + 死亡自罚 `-Level*5` + 谋杀恒 `AddBodyLuck(-500)`；等级目前是纯记账位（Delphi 仅 NPC 升级公式与 `nCHECKLUCKYPOINT` 读，均在脚本红线内），忠实追踪并持久化以备红线解除；②**`world/WeaponPoints`**：武器 per-instance `btValue[3]`(幸运)/`btValue[4]`(诅咒) 两字节；`MakeWeaponUnlock`（ObjBase.pas:2393）先耗幸运点再累积诅咒点（上限 10），复刻「已满仍发消息不变值」；**因 `RecalcAbilitys` 读 base catalog（`GetStdItem`）而非实例，故不入服务端战斗**，仅经 `GetItemAddValue`（ItmUnit.pas:124-125）折进客户端 `TClientItem`（武器 AC-low+=幸运、MAC-low+=诅咒，非武器不折）；③**`world/DisableTakeOffList`**：`InDisableTakeOffList`/`LoadDisableTakeOffList`（M2Share.pas:4642/4578），按物品名（小写）匹配（引擎以名为稳定身份，Delphi 以 StdItems.DB 索引），门禁 `ClientTakeOffItems`（ObjBase.pas:17144/17259）与 `DropUseItems`（ObjBase.pas:15532）跳槽；`isLockedInPlace` 由 static 改实例方法并叠加清单判定；`Mir2Server` 经 `MIR2_DISABLE_TAKEOFF_FILE`（GBK）装载并 `setDisableTakeOffList` 推入世界线程；④**战斗幸运暴击**：`GetAttackPower`（ObjBase.pas:2416）接入玩家近战——luck>0 有 `1/(10-min(9,luck))` 命中最大、luck<0 有 `1/(10-max(0,-luck))` 压到最小，玩家 luck=`bonus.luck-bonus.unLuck`（`EquipmentBonus` 已累加）；**关键：luck==0 逐位复刻旧 `randomBetween`（含 min==max 短路不抽签），确定性向量零回退**；怪物攻击不带 luck；⑤**持久化**：`character_state.body_luck REAL`、`character_equipment.weapon_luck/weapon_curse INTEGER`（均经 `ensureColumn` 原位加列，旧库默认 0），`PlayerState` 增第 7 分量、`BackpackItem` 增 `weaponPoints` 第 5 分量（均保留兼容构造器）；`WorldRandom.Stream` 增第 7 条流 `WEAPON_UNLOCK`（谋杀武器锁 roll 不扰动伤害序列） | `world/BodyLuck.java`、`world/WeaponPoints.java`、`world/DisableTakeOffList.java`（均新增）、`world/WorldEngine.java`（`makeWeaponUnlock`/`setDisableTakeOffList`/`isLockedInPlace` 实例化/`attackPower`/`playerLuck`/幸运值全链路）、`world/PlayerState.java`、`world/BackpackItem.java`、`world/WorldRandom.java`、`gate/ClientItemCodec.java`（幸运/诅咒折入客户端）、`persistence/SqliteStore.java`（三列）、`bootstrap/ServerConfig.java`/`Mir2Server.java`（`MIR2_DISABLE_TAKEOFF_FILE`）；新增 `BodyLuckTest`(8)+`WeaponPointsTest`(4)+`DisableTakeOffListTest`(6)+`WorldDisableTakeOffTest`(2)，扩 `ClientItemCodecTest`(+2)+`SqliteStoreTest`(+1)；**本地全量 307/307 绿**（283 → +24）。红线核对：未触碰技能/魔法、NPC 脚本引擎与 57 怪行为扩展。剩余延后项：`AddBodyLuck`/`WeaponPoints` 等级读取点（NPC 升级公式/`nCHECKLUCKYPOINT`）随脚本引擎红线一并延后、客户端拒绝回包 `无法取下物品`(M2Share.pas:3157) 未接、行会/沙城/FIGHT3 颜色分支仍延后 |
| ✅ 完成（本地闭环） | W23：虚拟时钟注入 + 会动的怪确定性对拍（需求方本轮显式指定） | ① **`world/WorldClock`（新增）**：三态时间源 `SYSTEM`（生产默认，`System.currentTimeMillis()`，与改动前逐位一致）/`VIRTUAL`（`epoch + ticks × tickMillis`，由引擎 tick 循环自增）/`MANUAL`（同上但只由调用方推进）；`DEFAULT_EPOCH_MILLIS=1_000_000` **不从 0 起**——`TBaseObject.Initialize`（ObjBase.pas:20095）用当前 tick 给冷却窗口打戳再按 `GetTickCount - stamp > interval` 判定，epoch=0 会让开局头几秒同时落在每个冷却窗口内；**已标注的刻意偏离**：Delphi 在 tick 内部实时读 `GetTickCount`，慢 tick 会让怪物一趟把「欠的」时间走完，虚拟模式量化到 tick 网格不累积欠时（50ms tick 下引擎所有区间 ≥200ms=≥4 tick，健康宿主不可观测，高负载下是真实偏离——故生产保持 `SYSTEM`）；② **`WorldEngine` 接线**：新增 `WorldClock` 字段 + 3 构造器重载（旧 `LongSupplier` 构造器全保留，24 处 `clock.getAsLong()` 一处未改）；`defaultHourSupplier` 让虚拟世界的昼夜小时也从自己的时钟取（`GetGameTime` 是第二个墙钟依赖，否则 `SM_DAYCHANGING` 对不齐）；`tickOnce()` 拆为「排空命令队列」+ `runTickBody()`（`TUserEngine.Run` 收包之后那半），`VIRTUAL` 在 tick body **之前**自增以保证本趟读到同一时间戳；新增 `advanceTicks(int)` 推进泵（走世界线程，每 tick 推一格时钟再跑完整 tick body，`SYSTEM`/`VIRTUAL` 上 no-op）；**MANUAL 下 `tickOnce()` early-return 跳过 `runTickBody()`**（命令排空仍无条件执行）——修掉的竞态：周期体相对入站命令的执行时机取决于宿主调度器，会让「怪物这一拳落在哪个 op 观测桶」变回赛跑；③ **`@tick [N]` GM 命令**（`processSay` → `pumpTicks`）：**无 Delphi 对应物**，harness 专用；仅 MANUAL 世界生效（`SYSTEM`/`VIRTUAL` 含所有生产服直接回绝，故运营不开手动时钟时该命令惰性）；上限 `MAX_PUMPED_TICKS=100_000`（50ms tick 下约 83 分钟虚拟时间，因一个泵 tick 是一整个 tick body，不设限则一行聊天可无限占住世界线程）；回执 `SM_SYSMESSAGE` `"@tick N -> T ticks, now=M"` **带新世界时间**，两侧 tick 数有出入会直接变状态差异而非静默漂移；④ **`ServerConfig`**：新字段 `worldClockMode`（默认 `SYSTEM`）+ `withWorldClockMode()` + `worldClock()` 工厂 + `MIR2_WORLD_CLOCK`（`system`/`virtual`，大小写不敏感，**`manual` 被环境解析显式拒绝**——运营设了会得到时间永不流动的世界，harness 走内部通道）；虚拟时钟 tick 间隔跟随 `worldTickMillis`，否则「tick 数相同」不再等于「世界时间相同」；⑤ **shadowdiff**：`--ai`（MANUAL 时钟 + 4 只 `chicken` + `Op.aiScript()`）、脚本 op `tick <N>`（与 `sleep` 的区别：`sleep` 让两侧墙钟各跑各的只是近似对齐，`tick` 把世界时间本身变成脚本量）、快照新增 `near=[(x,y) dir=N,...]`（视野内其它角色，排序去 id）与 `worldTime=`、`ShadowDiff` 增 `near:`/`worldTime:` 差异行、`--right-seed N` 负向对照开关、**报告新增「观测轨迹（左侧逐步状态）」表**（此前报告只列一致/差异判定，PASS 不可审计——分不清「18 步 0 差异」与「什么都没发生」；后续新增判定维度须同步进该表）；**修 bug**：`Args.parse` 漏把 `--ai` 登记为无值 flag 会吞掉下一参数（`--ai --monsters 4` 报 `unexpected argument: 4`），已抽出 `Args.FLAGS` 常量集 | `world/WorldClock.java`（新增）、`world/WorldEngine.java`（时钟字段/构造器族/`defaultHourSupplier`/`runTickBody` 拆分/`advanceTicks`/`pumpTicks`/`@tick` 派发/MANUAL early-return）、`bootstrap/ServerConfig.java`+`Mir2Server.java`（`MIR2_WORLD_CLOCK`）、`loadtest/LoadtestMain.java`、shadowdiff 六类（`ShadowDiffMain`/`ShadowSession`/`Op`/`ShadowDiff`/`StateSnapshot`/`ShadowReport`）；新增 `WorldClockTest`(7)+`WorldVirtualClockDeterminismTest`(6)+`ShadowDiffMainArgsTest`(3)，扩 `OpTest`(+2)+`ServerConfigTest`(+2)+`ShadowSessionEmbeddedTest`(+2)；**本地全量 329/329 绿**（307 → +22）。实跑证据见 `java-server/docs/g0-evidence/2026-09-23-w23-virtual-clock.md`：`--ai` 18 步 PASS（轨迹证明非空转）、`--right-seed 99999` 如期 FAIL（`#12 hit 6` dmg 2 vs 1）、决策边界对照墙钟 5/5 FAIL（差异数漂移 3/2/2/2/2）vs tick 版 5/5 PASS。**诚实的负面发现**：常规 `--ai` 脚本改用 `sleep` 的墙钟版在空载/6 进程满载/`--settle-ms 120` 下均 PASS（鸡的走 1400ms/攻 3000ms 节拍远大于本机抖动），必须构造决策边界场景才能稳定暴露差异——写结论时不夸大墙钟的不稳定性。红线核对：未触碰技能/魔法、NPC 脚本引擎与 57 怪行为扩展（`MonsterBehavior` 枚举与既有 AI 分支一行未改，`--ai` 用的是 W17 就已入库的 `chicken`）。剩余延后项：跨实现（Delphi↔Java）对拍会动的怪仍需 Delphi 侧补等价时间泵，或退回用「决策远离 op 边界」的脚本做墙钟对拍 |

| ✅ 完成（本地闭环 + 实跑证据） | W24：首批 10 种怪 AI 确定性对拍矩阵 | shadowdiff 新增 `--ai-all`，按固定白名单依次对拍 P2 已有十种怪；每行隔离双服/数据库、MANUAL 时钟、同种子，失败不短路；生成十份详细报告 + `ai-matrix.md` 汇总。实跑严格判定 10/10 PASS、180 步零差异；逐种轨迹证明非空转。未新增任何怪物行为 | `ShadowDiffMain.java`、`ShadowDiffMainArgsTest.java`；`docs/g0-evidence/2026-09-23-w24-first-ten-ai-matrix.md` |
| ✅ 完成（本地闭环 + 实跑证据） | W25：接入层尾项——IP 黑名单 + BlockMethod + 包体突发守卫 | ① **`gate/BlockIpList`（新）**：永久表复刻 `CompareLStr` **前缀匹配**（`203.0.113.` 封整段）、临时表 `CompareText` 全等；忠实复刻「条目长于待测地址则永不匹配」；`parse(Reader)` 走 RunGate 版 `LoadFromFile` **不做 `inet_addr` 校验**（网段前缀条目得以保留），持久化只写永久表（`SaveBlockIPList`）；② **`gate/BlockMethod`（新）** 三态 `DISCONNECT`/`BLOCK`/`BLOCK_LIST`，后两者写黑名单并 `CloseConnect(ip)` 踢掉该地址全部在线连接；③ **`gate/AccessPolicy`（重写）**：滑动窗口改为复刻原版 1s/3s **双 tumbling 窗**（`nIPCountLimit1=20`/`nIPCountLimit2=40`）+ 并发上限 `nMaxConnOfIPaddr=50`，黑名单判定先于限额，`Clock` 可注入；④ **`gate/PacketSizePolicy`（新）+ `ClientConnection` 守卫流**：复刻 RunGate 单次 recv 守卫（`>7000` 字节或 `>15` 帧即处置，`≤150` 字节不检查，`bokickOverPacketSize` 决定踢人或仅丢弃整块）——**仅 GAME 网关启用**，LoginGate/SelGate 读路径在原版中无任何校验；⑤ **`ServerConfig`** 新增 9 个环境变量并透传策略。**两处勘查修正**：(a) 并发上限**无 off-by-one**——被拒 socket 立即 Close 触发 `Dec(nCount)` 与先自增相互抵消，`limit=50` 恰好放行 50 条；Java 侧据此**在拒绝路径回退计数**，否则曾超限的 IP 会被永久锁死（该隐患由修正测试断言时暴露）；(b) 突发窗滚动时 `count:=0` 而非 1，故每窗实放 N+1 条，属原版 quirk 予以保留并在 Javadoc 标注。**负向对照**：把越界判定改为恒 `false` 后 5 个用例转红（357→352），确认用例非恒真断言，随后还原 | `gate/BlockIpList.java`、`gate/BlockMethod.java`、`gate/PacketSizePolicy.java`（均新增）、`gate/AccessPolicy.java`（重写）、`gate/GateServer.java`、`gate/ClientConnection.java`、`bootstrap/ServerConfig.java`+`Mir2Server.java`、`loadtest/LoadtestMain.java`；新增 `BlockIpListTest`(7)+`PacketSizePolicyTest`(9)+`GateAdmissionIntegrationTest`(7，**跨真实 TCP**)，重写 `AccessPolicyTest`(8)，扩 `ServerConfigTest`(+3)；**本地全量 362/362 绿**（330 → +32）。实跑证据 `docs/g0-evidence/2026-09-23-w25-gate-admission.md`（含并发上限 3/4 边界、包体 7000/7001 边界、`mBlock` 连带封禁与 `mDisconnect` 对照组，以及一处被协议解析污染的探针的纠正记录）。红线核对：未触碰技能/魔法、NPC 脚本引擎与 57 怪行为扩展 |
| ✅ 完成（本地闭环 + 实跑证据） | W26：组队管理与组队经验分摊 + 装备取下拒绝提示 | ① **`world/PlayerGroup`（新）**：组队数据模型（上限 12 人、队长管理、队员增删查、解散、单向只读快照）；② **`world/WorldEngine` 组队状态机与生命周期**：`setAllowGroup`（允许组队开关 `allowGroup`，关闭时自动退出当前队伍）、`createGroup`（创建队伍，严格校验发起人不在队中/目标在线且允许组队/目标未在队中）、`addGroupMember`（仅队长可邀请/12 人上限门禁）、`delGroupMember`（队长踢人/队员退队，队伍人数 $\le 1$ 时自动解散并通知 `SM_GROUPCANCEL`）、`leavePlayer`/`killPlayer` 触发掉线与死亡退队清理；③ **组队打怪经验分摊模型**：击杀怪物时按同图 12 格视距内（`max(|dx|, |dy|) <= 12`）存活队员数计算，2 人及以上享受 $1.0 + (N-1) \times 0.20$ 组队经验加成池，并按等级比例权重分摊四舍五入下发各队员 `SM_WINEXP`；单人或无队员在范围内则独享基础经验；④ **协议适配层（`gate/GameProtocolAdapter`）**：完整解码 `CM_GROUPMODE` (658)、`CM_CREATEGROUP` (659)、`CM_ADDGROUPMEMBER` (660)、`CM_DELGROUPMEMBER` (661)，编码分发 `SM_GROUPMODECHANGED` (658)、`SM_CREATEGROUP_OK` (659)、`SM_CREATEGROUP_FAIL` (660)、`SM_ADDGROUPMEMBER_OK` (661)、`SM_ADDGROUPMEMBER_FAIL` (662)、`SM_DELGROUPMEMBER_OK` (663)、`SM_DELGROUPMEMBER_FAIL` (664)、`SM_GROUPMEMBERSCHANGED` (665，组队名单换行拼接格式)、`SM_GROUPCANCEL` (666)；⑤ **W22 遗留提示补齐**：`TakeOffResult.CANNOT_TAKE_OFF` 触发下发 `SM_SYSMESSAGE` `"无法取下物品"` (M2Share.pas:3157)。红线核对：未触碰技能/魔法、NPC 脚本引擎与 57 怪行为扩展 | `world/PlayerGroup.java`（新增）、`world/WorldEngine.java`、`world/WorldEvent.java`、`gate/GameProtocolAdapter.java`；新增 `WorldGroupTest`(5)+`WorldGroupExpTest`(2)+`GameGroupProtocolTest`(3)，扩展 `GameItemProtocolTest`(+1)；**本地全量 371/371 绿**（362 → +9）。实跑证据见 `java-server/docs/g0-evidence/2026-09-24-w26-group-and-unequip-hint.md` |

### 当前下一步（Next Session）

> **W26 已交付（2026-09-24）：**组队管理系统（12人上限、创建/加人/踢人/退队/解散/离线死亡清理）、组队打怪经验加成与按等级权重分摊模型（12格视距判定、20%递增加成池）、完整组队 CM/SM 协议栈（658-666）及装备不可取下拒绝提示全量落地，本地 **371/371 绿**（+9）。实跑证据见 `java-server/docs/g0-evidence/2026-09-24-w26-group-and-unequip-hint.md`。**下一次开发优先级**：①**Netty 接入替换**（接入层最后一块，W02 起挂账；当前 `GateServer` 为 thread-per-connection + 虚拟线程，替换前需先确认是否仍有必要）；②**env-lint / 配置校验收口**（把 fail-fast 校验推广到全部 `MIR2_*`）；③**行会系统**（P3 独立候选，`GetGuildRelation` / 战盟敌对颜色等分支，动工前需重新确认范围）；④技能/魔法与 NPC 脚本引擎**红线仍未解除，不得开始**。G3 仍未正式过门：①3 名老玩家盲测、②Delphi↔Java 十怪执行，共同卡真实环境（Windows + mir2.exe + Delphi 服务端）。

> **上一步（W25，已完成）：**接入层尾项三件（IP 黑名单 / `BlockMethod` / 包体突发守卫）全量落地，本地 362/362 绿（+32），含 7 个跨真实 TCP 的集成用例与一组负向对照。Delphi 侧时间泵勘查已完成且结论为否定，G3 ② 的跨实现动态 AI 对拍改走「墙钟但远离决策边界」路线。

> **W23 已交付（2026-09-23）：**「虚拟时钟注入 + 会动的怪确定性对拍」全量落地（见任务表 W23 行）：`world/WorldClock` 三态时间源把 Delphi 遍布全服的 `GetTickCount` 收敛为可注入依赖，`VIRTUAL`/`MANUAL` 下所有节拍变成 tick 计数的纯函数；`WorldEngine` 拆 `runTickBody()` + `advanceTicks(int)` 推进泵 + `@tick [N]` harness 专用命令（MANUAL 下 `tickOnce()` 跳过周期体以消除观测桶归属竞态）；`MIR2_WORLD_CLOCK`（`system` 默认 / `virtual`，`manual` 拒绝）；shadowdiff `--ai` + `tick` op + `near=`/`worldTime=` 快照 + `--right-seed` 负向对照 + 「观测轨迹」报告表；顺手修掉 `Args.parse` 吞参数 bug。本地 329/329 绿（+22）。实跑结论：`--ai` 18 步 PASS 且可审计非空转、`--right-seed 99999` 如期 FAIL、决策边界对照墙钟 5/5 FAIL vs tick 版 5/5 PASS。**G3 门「10 种怪 AI 对拍一致」的工具前提至此就绪**，但跨实现对拍仍卡两点：真实客户端/Delphi 环境，以及 Delphi 侧没有 `@tick` 等价的时间泵（remote `--ai` 要求两侧都跑手动时钟，非生产配置）。**下次开发起点（W24）候选优先级**：①**把 `--ai` 扩到 10 种怪**（G3 门②的直接剩余工作量：`MonsterTemplate` 现有 10 种怪 + 木桩已全部由 W18 的真实 Monster.DB 构建，逐种跑 `--ai --monster-kind <种>` 并把结果表入库证据文档；**注意红线**——只对已入库的 10 种怪做对拍，不得为此新增 57 种怪物行为中的任何一种）；②**Delphi 侧时间泵可行性勘查**（只读勘查不动工：评估在 `TUserEngine.Run` 外挂一个可被 GM 命令驱动的 tick 泵的改动面，决定跨实现对拍会动的怪是否值得；若判为不值得，则退回用「决策远离 op 边界」的慢节拍怪脚本做墙钟对拍，W23 已证明该做法在鸡上可行）；③**行会系统**（`GetGuildRelation` / `btAllyAndGuildNameColor $B4` / `btWarGuildNameColor $45` 三条颜色分支自 W20 起显式延后，行会本身是 P3 独立切片）；④技能/魔法切片（P3 首块，**红线仍在，须需求方明确解除后方可动工**）；⑤NPC 脚本引擎（`Market_Def`，**红线仍在**）；⑥W22/W23 剩余尾项：客户端拒绝回包 `无法取下物品`(M2Share.pas:3157) 接线、`BodyLuck`/`WeaponPoints` 等级读取点随脚本引擎一并落地。G3 门状态：②的工具前提已由 W23 补齐，①「3 名老玩家盲测手感无差异」与②的跨实现执行仍共同卡真实客户端环境（Windows + mir2.exe + Delphi 服务端），环境就绪后 wiretool 录 golden + shadowdiff 零改码对拍。红线提醒不变：57 种怪物行为、技能/魔法、NPC 脚本引擎依然自动禁止（W23 未触碰：`MonsterBehavior` 枚举与既有 AI 分支一行未改）。

> **上一步（W22，已完成）：**「补齐 W20 遗留小项」全量落地（见任务表 W22 行）：`world/BodyLuck` 幸运值累加器（`AddBodyLuck` 非对称守卫 + 经验/升级/死亡/谋杀全链路）、`world/WeaponPoints` 武器幸运/诅咒双字节（`MakeWeaponUnlock` 先耗幸运再累积诅咒、仅折进客户端 `TClientItem` 不入服务端战斗）、`world/DisableTakeOffList`（`InDisableTakeOffList` 门禁取下与死亡掉落、`MIR2_DISABLE_TAKEOFF_FILE` GBK 装载）、`GetAttackPower` 幸运暴击接入玩家近战（**luck==0 逐位复刻旧抽样、确定性向量零回退**），三列持久化（`body_luck`/`weapon_luck`/`weapon_curse` 原位加列）。本地 307/307 绿（+24）。**下次开发起点（W23）候选优先级**：①**虚拟时钟注入 + 会动的怪确定性对拍**（直接服务 G3 门「10 种怪 AI 对拍一致」；注意会偏离「真实进程对拍」初衷，动工前需权衡）；②技能/魔法切片（P3 首块，**红线仍在，须需求方明确解除后方可动工**）；③**行会系统**（`GetGuildRelation` / `btAllyAndGuildNameColor $B4` / `btWarGuildNameColor $45` 三条颜色分支现被 W20 显式延后，行会本身是 P3 独立切片）；④NPC 脚本引擎（`Market_Def`，**红线仍在**）——W21 只交付 NPC 可见站位，对话/交易须解除红线后方可动工；⑤W22 剩余尾项：客户端拒绝回包 `无法取下物品`(M2Share.pas:3157) 接线、`BodyLuck`/`WeaponPoints` 等级读取点随脚本引擎一并落地；⑥README/MVP 草案等冗余收尾确认。G3 门状态不变：盲测手感与 10 怪对拍的共同前置仍是真实客户端环境（Windows + mir2.exe + Delphi 服务端），环境就绪后 wiretool 录 golden + shadowdiff 零改码对拍。红线提醒不变：57 种怪物行为、技能/魔法、NPC 脚本引擎依然自动禁止（W21/W22 均未触碰：W21 NPC 仅可见站位无脚本、W22 无新怪物行为/无技能/无脚本引擎钩子）。

> **上一步（W20/W21，已完成）：**W20 交付「`DropUseItems`（死亡掉装备）+ PK 等级/名字颜色模型 + 红名全掉」（装备两趟扫槽、三面护身旗、`$FB/$F9/$2F` 名字颜色、2 分钟衰减、60 秒 `SetPKFlag`、谋杀 +100 点、`boDieRedScatterBagAll` 红名全掉，并纠正 `ScatterBagItems` 误用 `NOTHROWITEM` 的地图旗），本地 276/276 绿；W21（PR #39 合入 master）交付 NPC 最小切片可见半（`spawnNpc`/`MIR2_NPC_LIST`）与真机四联修复（`CM_SOFTCLOSE`/`SM_CHANGELIGHT`/`reconcileStandardItems`），基线抬到 283/283。

> **上次开发起点（W18/W19，已完成）：**W18 交付真实 StdItems.DB / Monster.DB 导入（纯数据、不受红线），W19 把被并进 `NeedIdentify` 的 `TStdItem.Reserved` 拆回独立字节并复刻 `Reserved & 2`/`& 4` 两条取下锁定 quirk——这两项正是 W20 `DropUseItems` 读 `Reserved and 8`/`and 10` 的前置。

> **更早起点（W17→W18 交接，存档）：**W17 已交付世界随机种子注入 + 带怪 PvE 影子对拍，**W16 遗留的「embedded 自拍不放怪」已知限制就此解除**：`WorldRandom` 把随机性按用途拆成 5 条互不干扰的流（掉落 roll 不再扰动伤害序列），`MonsterBehavior.STATIONARY` + `trainer()` 木桩提供唯一行为与墙钟无关的 PvE 目标，shadowdiff 观测面扩到 `SM_STRUCK/SM_DEATH/SM_WINEXP`（伤害、经验、死亡），CLI `--pve --seed` 一行跑通，CI 已加第二道门禁。负向对照已证明非空转：仅种子不同即 FAIL（6 处 `dmg` 差异），同种子 PASS（0 差异）。**G3 门现状不变**：①「3 名老玩家盲测手感无差异」②「10 种怪 AI 对拍一致」的共同前置仍是**真实客户端对拍环境（Windows + mir2.exe + Delphi 服务端）**，这是全项目最大缺口；环境就绪后零改码：wiretool `record` 录 Delphi golden + shadowdiff `--left-host <Delphi LoginGate> --right-host <Java>`（PvE 对拍再各配同一 `MIR2_WORLD_SEED`）。**优先建议**：① **真实 `StdItems.DB`/`Monster.DB` 导入**——替换 W04/W09/W12/W15/W17 的全部 `TODO(verify)` 数值（含本次木桩 HP/AC）；**纯数据工作、不受红线约束，且能立刻提高对拍含金量**，仍是性价比最高的下一件；② **会动的怪确定性对拍**（中件）：当前木桩之外的怪物仍会因两进程墙钟差异发散，需要给世界注入**虚拟时钟**（tick 计数替代 `System.currentTimeMillis()`）才能对拍 AI 走位——注意这会偏离「真实进程对拍」的初衷，动工前需权衡；③ **技能/魔法切片**（P3 首块：`CM_SPELL`、`TUserMagic`、`SM_MAGICFIRE`、`Magic.pas` 59 技能框架）——**红线仍在，须需求方明确解除后方可动工**；④ W14 遗留 `TODO(verify)`：`DropUseItems`（死亡掉装备）依赖 `StdItem.Reserved` 位与 PK 等级模型，红名 `boDieRedScatterBagAll` 全掉分支同理。可并行项不变：bot-swarm 跨图连接点走图/更高并发、城堡门差异分支、Netty 接入替换。红线提醒：**57 种怪物、技能/魔法、NPC 脚本引擎的红线依然有效**（W12 仅解除装备穿脱/使用/丢弃三项；W15 修理走协议状态机未触碰 Market_Def；W16 影子对拍是纯工具件；**W17 木桩是 monster 对象而非 NPC，未触碰脚本引擎，且只是第 11 个模板、不构成 57 怪全量扩展**）。除鸡/半兽人外 8 种怪物模板数值、W12 装备属性、W15 复活戒指、W17 木桩数值均为 TODO(verify) 占位，Monster.DB/StdItems.DB 导入前不视为权威。历史条目见任务表各行。
...
| 影子对拍 harness：同操作流双服 diff 状态快照 ✅（W16 交付 `shadowdiff` 模块；W17 补世界种子注入 + 带怪 PvE 对拍与伤害/经验观测面；Delphi 侧接入只差环境） | `—` | **B** | 2 |
...
| `v1.0.25` | `2026-09-23` | 交付 **W20 `DropUseItems`（死亡掉装备）+ PK 等级/名字颜色模型 + 红名全掉**，补齐自 W14 起挂账最久的死亡惩罚缺口。① **`TPlayObject.DropUseItems`**（ObjBase.pas:15487）两趟扫十三槽：趟一 `Reserved and 8` **原地销毁、永不落地**，且以 `DelList.AddObject('', MakeIndex)` 的**空名**回报（客户端只认 MakeIndex）——`StdItem` 紧凑构造器禁止空名，故新增 `world/ItemRemoval` 记录承载 `SM_DELITEMS` 条目、`WorldEvent.ItemsRemoved` 随之改签名；趟二每槽 `Random(nRate) = 0`（`nDieDropUseItemRate` 30，`PKLevel > 2` 取 `nDieRedDropUseItemRate` 15），中签 `DropItemDown(...,2,True)` 落地后**只有 `Reserved and 10 = 0` 才清槽**——`Reserved and 2` 物品「掉在地上却仍然穿着」的复制 quirk 原样照搬；② **门禁链**：`m_boAngryRing or m_boNoDropUseItem` 先退，`Die`（21002）再按 `boKillByMonstDropUseItem`(True)/`boKillByHumanDropUseItem`(False) 判凶手——怪物杀掉装备、玩家杀不掉、无归属击杀必掉；**`DropUseItems` 无地图门禁**（`Die` 的 `(not m_boNoItem) or (not Flag.boNODROPITEM)` 是 OR，纯 NODROPITEM 图满足不了），忠实保留；③ **`world/DropProtection`** 复刻 `RecalcAbilitys` 三面护身旗 `m_boAngryRing`/`m_boNoDropItem`/`m_boNoDropUseItem`，**槽位不对称保留**（武器/右手/衣服读 AniCount、其余读 Shape；码值 170/171/172；经典 117 只在手衣分支与两戒指槽生效；0 耐久不生效）；④ **`world/PkLevel`**：`PKLevel = m_nPkPoint div 100`，255 白 → $FB 黄 → $F9 红，**顺序 quirk**：$2F 交手色压过等级色但只在 `PKLevel < 2` 时；⑤ **计数器与广播**：谋杀 `IncPkPoint` +100，`TBaseObject.Run` 每 2 分钟 `DecPKPoint` -1，**广播 quirk：等级变化且旧等级为 1 或 2 才 `RefNameColor`**（深红 3→2 全程不重绘）；`SetPKFlag`(21220) 把**攻击方**染色 60 秒（整块以受害者 `not m_boPKFlag` 为前置，任一方红名或 FIGHT 区跳过），`CheckPKStatus` 到点清除；新事件 `WorldEvent.NameColorChanged` → `RM_CHANGENAMECOLOR(8090)` → `SM_CHANGENAMECOLOR(656)` 广播给全体观察者与本人；⑥ **谋杀分支**（20938，出厂配置：`boKillHumanWinLevel/Exp` 皆 False、无行会战无沙城）：凶手 +100 点，双方收 GBK「你犯了谋杀罪...」/「你被%s杀害了...」；受害者已挂旗 → `IsGoodKilling` 只发「[--你受到正当规则保护--]」不计点；FIGHT 区或受害者已红名整段跳过；`AddBodyLuck(-500)`/`MakeWeaponUnlock` 缺幸运与武器锁模型，注释标注延后；⑦ **`boDieRedScatterBagAll`**：`PKLevel >= 2` 死亡包裹**全掉**而非 1/3；⑧ **顺带纠错**：`ScatterBagItems` 长期误用 `boNOTHROWITEM` 作豁免旗——Delphi 里禁丢(`NOTHROWITEM`)与禁死亡掉落(`NODROPITEM`)是两个旗，`MapFlags`/`MapInfoLoader` 补 `NODROPITEM` 解析；`WorldRandom.Stream` 增第 6 条 `DEATH_DROP_USE_ITEM`（装备掉落不再扰动包裹序列）；⑨ **持久化**：`character_state.pk_point` 原位加列（旧库默认 0）、`PlayerState` 增第 6 分量（旧构造器保留兼容重载）、`m_boPKFlag` 按 Delphi 语义**不落库**。新增 `WorldPkAndEquipmentDropTest`(17)，**本地全量 276/276 绿**（259 → +17）。**红线核对**：未触碰技能/魔法、NPC 脚本引擎、57 怪行为扩展。剩余延后：`InDisableTakeOffList`（配置文本清单）、`AddBodyLuck(-500)`、`MakeWeaponUnlock`、行会/沙城/FIGHT3 颜色分支 |
| `v1.0.24` | `2026-09-23` | 交付 **W19 `TStdItem.Reserved` 独立字节还原 + 取下锁定 quirk**（PR #37）：W18 导入时 `RESERVED` 列被并进 `NeedIdentify`，本轮拆回 `StdItem.reserved` 独立字段并对 W18 旧缓存做 SQLite 原位迁移；复刻 `ClientTakeOffItems`（ObjBase.pas:17238-17262）两条取下禁制——`m_boUserUnLockDurg = False`（登录默认）时 `Reserved & 2` 锁定、`Reserved & 4` 无条件锁定；祈祷/赤血类标志在 `TClientItem` 线上保真。本地 259/259 绿（253 → +6）。该字节正是 W20 `DropUseItems` 读 `Reserved and 8`/`and 10` 的前置 |
| `v1.0.23` | `2026-09-22` | 交付 **W18 真实 StdItems.DB / Monster.DB 导入**（W17 交接建议①，纯数据、不受红线约束）：① **数据源**：官方 1.76 GEEM2 基线转储（`cjlaaa/Mir2-GeeM2` main @9981a8d7，`数据库/GEEM2.db.sql` + `Envir/MonItems/*.txt`），与 W09 外观校正同源；新增 `java-server/scripts/extract-geem2-db.py` 确定性生成受管工件（防呆：名称 ≤20 GBK 字节与 Delphi `Char[20]` 一致、重名按 Delphi 首行命中收敛、非 1 分子告警、金币行单列）；② **受管工件**：`world/resources/db/MonsterDb.tsv`（378 行全表，22 原生列）、`StdItemsDb.tsv`（686 行全表，两个上游重名首行获胜收敛为 684 项）、`MonItems/`（10 张 ascii 文件名 + `index.tsv` 名称间接层——中文资源名在 C locale JVM 下 `sun.jnu.encoding=ANSI` 无法寻址）；③ **加载器**：`MonsterDb`（`clampActionInterval` 复刻 `LoadMonsterDB` 的 200ms 下限 + `monsterWeapon=LoByte(MP)`）、`StdItemsDb`（Ac/Ac2…Sc/Sc2 打包 Delphi dword，RESERVED→needIdentify）、`MonsterDropTable`（`金币` 行解析为 `GoldDrop` 延期——金币地面堆/拾取入钱包属 P3，未落地；`FileExists` 语义：无表怪物空掉落）；④ **接线**：`MonsterTemplate` 首批 10 种怪 + 木桩全部 `fromDb`（`UsrEngn.pas:2581` 映射：HP=MaxHP=wHP、AC=MakeLong(wAC,wAC) 平直值、DC=MakeLong(DC,DCMAX)、Level/Exp、走攻间隔=WALK_SPD/ATTACK_SPD 钳制后值；掉落换真实 MonItems——**W03 占位 `鹿肉` 退役**（真库只有 `肉`），`洞蛆` 真掉落含 9 种特戒各 1/1000 万）；`StdItems.defaults()` 升级为 684 项权威全量导入（W04 占位与全部 TODO(verify) 消除：`鸡肉` StdMode40/DuraMax4000、`木剑` Shape1/Weight4/DuraMax4000/DC2-5/NeedLevel1/价50、`金创药(小量)` 价40，`复活戒指` AC0-1/MAC0-1/Looks175/NeedLevel16/价20000），SQLite `std_items` 启动种子自动跟随；⑤ **测试**：新增 `Geem2BaselineImportTest`(5) 钉死模板↔DB↔掉落全字段对齐 + 重名首行获胜 + 解析器拒绝坏行；7 处既有断言按权威数值更新（木剑负重/字段、鸡肉 Looks 13、鸡 exp=9 → 12 杀升级余 8、半兽人攻击间隔 2500ms → 三处时间推进改以 `attackIntervalMillis()` 推导、占位名「屠龙」真库已存在故改「根本不存在之剑」）；**本地全量 253/253 绿**（248 → +5）。**红线核对**：未新增怪物行为（57 怪红线）、未触碰技能/魔法与 NPC 脚本、Magic 表未导入。剩余占位：视野范围/行为族常数仍为代码 TODO(verify)（Monster.DB 无该列）、`金币` 掉落延期 P3 |
| `v1.0.22` | `2026-09-22` | 交付 **W17 世界随机种子注入 + 带怪 PvE 影子对拍**，解除 W16 交付时记录的「embedded 自拍不放怪」已知限制。① **`WorldRandom`**：`of()` 复刻 Delphi 全服共用一个全局 `Random()`（未设种子 = 生产默认，语义与原版一致）；`seeded(seed)` 按用途拆 **5 条互不干扰的流**（`DAMAGE`/`EQUIPMENT_WEAR`/`LOOT_DROP`/`DEATH_SCATTER`/`SPAWN`），各由种子经 **SplitMix64 雪崩**派生。**为什么必须拆流**：Delphi 全局单序列下，一次掉落抽样会挪走本该属于下一次伤害的抽样；两个对拍进程的怪物 AI/刷怪计时/HP 回复都由**墙钟**驱动、抽样次数永远对不齐，于是后续所有数值错位——拆流后掉落 roll 几次不再影响第 N 次伤害；② `WorldEngine` 9 处随机点改走对应流 + `worldSeed()`；`ServerConfig`/`Mir2Server` 增 `MIR2_WORLD_SEED`（未设 = 生产随机）；启动日志打印 `worldSeed=<值|unseeded>`；③ **`MonsterBehavior.STATIONARY` + `MonsterTemplate.trainer()`（木桩）**：忠实复刻 `TMonster.Run` 的 **`if not m_boNoAttackMode`（ObjMon.pas:449）**——该条件整块包住「索敌→追击→攻击」，Java 侧 tick 循环开头 `continue`。溯源 `TRAINER=55`（M2Share.pas:152）→ `TTrainer`（ObjNpc.pas:2626）官方伤害测试木桩（挨打回报破坏力/平均值）；Delphi 侧为 NPC，本次因 **NPC/脚本引擎红线未解除**改用 monster 对象实现等价线上可观测行为（站立/挨打/死亡/不掉落），**不引入脚本钩子**；它是**唯一行为与墙钟无关**的 PvE 目标，故可确定性对拍。仓内 Delphi 源与 ini 均无「木人/稻草人/训练」中文串，「木桩」为自拟译名；HP 5000/AC 为 `TODO(verify)`，待 `Monster.DB` 第 55 行校准；④ **shadowdiff 观测面扩展**：`StateSnapshot` 增 `experience`/`combat` 字段（保留旧 12 参兼容构造器），`ShadowSession` 新解析 `SM_STRUCK`（伤害值 + 受击方 HP，对象 id 归一化为 self/other）、`SM_DEATH`、`SM_WINEXP`（总经验 + 增量），`TAbility` 解包补读 exp，`ShadowDiff` 增 `exp:`/`combat:` 比较——**此前挨打掉血完全不在对拍视野内**；⑤ CLI 增 `--pve`（内置 PvE 脚本：贴桩走位 + 6 连击含 heavyhit/bighit + relog 存档往返，每击后 `sleep 1000` 清 900ms `CM_HIT` 间隔）、`--monsters N`、`--monster-kind NAME`、`--seed N`（默认 20260921，两侧共用；remote 模式两台服务端各配同一 `MIR2_WORLD_SEED`）；⑥ CI `shadowdiff-smoke` 增第二道门禁 `--embedded --pve --seed 20260922`，报告随 artifact 上传。新增 `WorldRandomTest`(6，含流间独立/`between` 退化区间不消耗)、`WorldSeedDeterminismTest`(5，含掉落不扰动伤害流、木桩 40 tick 不移动不还手、1HP 木桩可击杀)、`ServerConfigTest` +2、`ShadowSessionEmbeddedTest` +2（真 TCP 双服带怪 PvE + 异种子发散检测），**本地全量 237/237 绿**（222 → +15）。**实跑证据** `g0-evidence/2026-09-22-shadowdiff-pve-seeded.md`：严格判定 17/17 步 PASS；**负向对照**——同脚本同地图同 8 木桩仅种子不同 → unseeded **FAIL（6 处状态差异）**、seeded **PASS（0 差异）**，证明对拍非空转。**明确未解决**：会动的怪仍因墙钟差异不可确定性对拍（需虚拟时钟注入，会偏离真实进程对拍初衷）；G3 剩余两项仍卡真实客户端环境。下一步：真实 StdItems.DB/Monster.DB 导入（纯数据、不受红线，性价比最高）、虚拟时钟（权衡后再定）、技能切片（红线未解除，禁动） |
| `v1.0.21` | `2026-09-21` | 交付 **W16 影子对拍 harness**（P2 任务表最后一个未启动项，G3「影子对拍工具可用」硬性要求）：新增 `java-server/shadowdiff` 模块（shaded `mir2-shadowdiff.jar`，dist 通道第四 JAR）。① `Op` 脚本方言：一行一 op（`#` 注释），`turn/walk/run/hit/heavyhit/bighit <0..7>`、`pickup`、`bag`、`say`、`drop/eat/takeon/takeoff <物品名>`、`sleep <ms>`、`relog`（存档往返），内置 29 步冒烟脚本；② `ShadowSession` 走真实三端口线上协议（`#…!` 帧 + RunLogin + `+GOOD/+FAIL`，与 bot-swarm 同方言、独立实现），三段式登录 + 自动建号 + 进图重试，每 op 后按静默窗口（默认 400ms）排干应答并从线上包重建 `StateSnapshot`（地图/坐标/朝向/HP/MP/等级/金币/背包/装备；**MakeIndex/对象 id 归一化**，物品身份 = 名称+Dura/DuraMax 排序集合）；解析 50B `TAbility`/76B `TClientItem`/`SM_SENDUSEITEMS`/`SM_DELITEMS`；③ `ShadowDiff` 三级判定：STATE/ACKS 差异 = FAIL，MESSAGES（消息多重集合，顺序归一化）默认仅提示、`--strict-messages` 升级 FAIL；④ `ShadowReport` 中文 Markdown+CSV，退出码即结论；⑤ CLI：`--embedded` 进程内双 Java 自拍（独立 SQLite/端口）= 骨架自检 + 确定性回归；remote `--left-host/--right-host` 对拍任意两台——**Delphi 环境就绪后指向 Delphi LoginGate 即为 G3 对拍，零改码**；⑥ CI 增 `shadowdiff-smoke` 每 PR 门禁（宽松消息判定：顺序会话可能跨昼夜边界）；⑦ 已知限制：embedded 自拍不放怪（双世界各自独立 Random，PvE 数值对拍需后续世界种子注入切片）。新增 `OpTest`(6)+`ShadowDiffTest`(7)+`ShadowReportTest`(2)+`ShadowSessionEmbeddedTest`(3，真 TCP 双服含发散检测)，**本地全量 222/222 绿**（204 → +18）；embedded 严格判定 29/29 步 PASS，报告入库 `g0-evidence/2026-09-21-shadowdiff-embedded-strict.md`。**P2 任务表全部收口**；G3 剩余：盲测手感 + 10 怪对拍，均卡真实客户端环境。下一步：真实 StdItems.DB/Monster.DB 导入（纯数据、不受红线）、世界随机种子注入（解锁带怪对拍）、技能切片（红线未解除，禁动） |
...
| `v1.0.28` | `2026-09-23` | 交付 **W24 首批 10 种怪 AI 确定性对拍矩阵**：shadowdiff 新增 `--ai-all`，以显式白名单固定 P2 十种既有模板，每种隔离双服/SQLite、MANUAL 时钟与同种子，失败行不中断矩阵，输出逐种详细报告与汇总表；严格模式实跑 10/10 PASS、180 步状态/应答/消息零差异，逐种 `near` 轨迹及受击/逃跑观测排除空转。未新增怪物行为；G3 跨实现签发仍待 Delphi 环境。 |
| `v1.0.29` | `2026-09-23` | 交付 **W25 接入层尾项**：`BlockIpList`（永久前缀 / 临时全等两级黑名单，复刻 `CompareLStr` 与 RunGate 无校验装载）、`BlockMethod` 三态处置（含连带踢连）、`PacketSizePolicy` + 守卫输入流（单次 recv `>7000` 字节或 `>15` 帧，仅 GAME 网关）、`AccessPolicy` 重写为 1s/3s 双 tumbling 窗 + 并发上限，`ServerConfig` 新增 9 个环境变量。勘查修正两处：并发上限**无 off-by-one**（拒绝路径须回退计数，否则超限 IP 永久锁死），突发窗 N+1 属原版 quirk 保留。本地 362/362 绿（+32，含 7 个跨真实 TCP 用例 + 负向对照 5 例转红）。**Delphi 时间泵勘查结论为否定**（775 处裸 `GetTickCount`），G3 ② 改走墙钟远离决策边界路线。未触碰红线。 |
| `v1.0.30` | `2026-09-24` | 交付 **W26 组队管理与经验分摊 + 装备不可取下拒绝提示**：① `PlayerGroup` 数据模型（12人容量、队长与成员管理、解散、单向快照）；② `WorldEngine` 完整组队状态机（`setAllowGroup`/`createGroup`/`addGroupMember`/`delGroupMember` 及掉线与死亡自动退队/解散）；③ 组队经验分摊与加成模型（12格视距内 2人以上按 $1+(N-1)\times 20\%$ 加成并按等级比例权重分摊，下发各队员 `SM_WINEXP`）；④ 网关协议适配 `CM_GROUPMODE` (658)、`CM_CREATEGROUP` (659)、`CM_ADDGROUPMEMBER` (660)、`CM_DELGROUPMEMBER` (661) 与相应 SM 回包（658-666）；⑤ W22 遗留 `无法取下物品` 提示补齐。本地 **371/371 绿**（+9）。未触碰技能/魔法与脚本引擎红线。 |
...
*📋 MIR2 → JAVA · DEVELOPMENT PLAN v1.0.29**
