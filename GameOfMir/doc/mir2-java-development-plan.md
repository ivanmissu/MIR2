...
*Development Plan · v1.0.25 · 2026-09-23*
...
> **W20 已交付（2026-09-23）：**「`DropUseItems`（死亡掉装备）+ PK 等级/名字颜色模型 + 红名全掉」全量落地（见任务表 W20 行与 §1 执行状态），**W14 起挂账最久的死亡惩罚缺口就此补齐**：装备两趟扫槽（`Reserved and 8` 空名销毁 / 1-in-30、红名 1-in-15 掉落 / `Reserved and 10 <> 0` 掉了还穿的复制 quirk）、三面护身旗 `DropProtection`、`PKLevel` 与 `$FB/$F9/$2F` 名字颜色（交手色只在红名以下压过等级色）、2 分钟衰减（**旧等级 1 或 2 才重绘**）、60 秒 `SetPKFlag`、谋杀 +100 点与两句 GBK 文案（受害者挂旗判正当防卫）、`boDieRedScatterBagAll` 红名全掉，并纠正 `ScatterBagItems` 误用 `NOTHROWITEM` 的地图旗（真旗是 `NODROPITEM`）。本地 276/276 绿。**下次开发起点（W21）候选优先级**：①**虚拟时钟注入 + 会动的怪确定性对拍**（直接服务 G3 门「10 种怪 AI 对拍一致」；注意会偏离「真实进程对拍」初衷，动工前需权衡）；②技能/魔法切片（P3 首块，**红线仍在，须需求方明确解除后方可动工**）；③**行会系统**（`GetGuildRelation` / `btAllyAndGuildNameColor $B4` / `btWarGuildNameColor $45` 三条颜色分支现被 W20 显式延后，行会本身是 P3 独立切片）；④本轮延后的小项：`InDisableTakeOffList`（服务器配置文本清单）、`AddBodyLuck(-500)` 幸运模型、`MakeWeaponUnlock` 武器锁；⑤README/MVP 草案等冗余收尾确认。G3 门状态不变：盲测手感与 10 怪对拍的共同前置仍是真实客户端环境（Windows + mir2.exe + Delphi 服务端），环境就绪后 wiretool 录 golden + shadowdiff 零改码对拍。红线提醒不变：57 种怪物行为、技能/魔法、NPC 脚本引擎依然自动禁止（W19/W20 均未触碰：无新怪物行为、无技能、无脚本引擎钩子）。
