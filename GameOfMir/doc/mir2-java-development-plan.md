...
*Development Plan · v1.0.21 · 2026-09-21*
...
> **下次开发起点（W17）：**W16 已交付影子对拍 harness（`shadowdiff` 模块，embedded 双 Java 自拍严格判定 PASS、发散检测验证非空转、CI 每 PR 门禁、dist 第四 JAR）——**P2 任务表全部行至此收口**。**G3 门（P2 收口）现状评估：工具侧三件套（bot 军团 / wiretool / shadowdiff）齐活，但仍未正式过门**——①「3 名老玩家盲测手感无差异」②「10 种怪 AI 对拍一致」两项的共同前置仍是**真实客户端对拍环境（Windows + mir2.exe + Delphi 服务端）**，这是全项目最大缺口；环境就绪后的动作已完全就绪：wiretool `record` 录 Delphi golden + shadowdiff `--left-host <Delphi LoginGate> --right-host <Java>` 直接跑同脚本对拍，两者均无需改代码。**优先建议**：① 待需求方提供真实 `StdItems.DB`/`Monster.DB` 导出后，替换 W04/W09/W12/W15 的全部 `TODO(verify)` 数值（这是纯数据工作，不受红线约束，且能立刻提高对拍的含金量）；② **世界随机种子注入切片**（小件）：给 `WorldEngine` 的 `Random` 开一个可配置种子（`MIR2_WORLD_SEED`），让 embedded 影子对拍可以在**有怪物**的世界里对拍 PvE 伤害/掉落——当前 embedded 自拍不放怪即为此限制；③ **技能/魔法切片**（P3 首块：`CM_SPELL`、`TUserMagic`、`SM_MAGICFIRE`、`Magic.pas` 59 技能框架）——**红线仍在，须需求方明确解除后方可动工**；④ W14 遗留 `TODO(verify)`：`DropUseItems`（死亡掉装备）依赖 `StdItem.Reserved` 位与 PK 等级模型，红名 `boDieRedScatterBagAll` 全掉分支同理。可并行项不变：bot-swarm 跨图连接点走图/更高并发、城堡门差异分支、Netty 接入替换。红线提醒：**57 种怪物、技能/魔法、NPC 脚本引擎的红线依然有效**（W12 仅解除装备穿脱/使用/丢弃三项；W15 修理走协议状态机未触碰 Market_Def；W16 影子对拍是纯工具件，不涉玩法扩展）。除鸡/半兽人外 8 种怪物模板数值、W12 装备属性与 W15 复活戒指数值均为 TODO(verify) 占位，Monster.DB/StdItems.DB 导入前不视为权威。历史条目见任务表各行。
...
| 影子对拍 harness：同操作流双服 diff 状态快照 ✅（W16 交付 `shadowdiff` 模块；Delphi 侧接入只差环境） | `—` | **B** | 2 |
